...
% Last Modified by GUIDE v2.5 13-Feb-2018 10:50:58
...
if length(varargin)>5
    ...
    DataStruct.minSacc=0.04;
    DataStruct.minVelSacc=0.0003;
    DataStruct.timeSP=5;
...
DataStruct.velth=0.0005; %threshold to detec saccades
...
% uiwait(hObject);
set(handles.vel_Axes,'nextplot','add');ylabel('Derivatives of dF/F0 [(dF/F0)/s]')
% uiwait(hObject);
%
if isfield(DataStruct,'rawtime')
...
% uiwait(hObject);
set(handles.pos_Axes,'nextplot','add');ylabel('Derivatives of dF/F0 [(dF/F0)/s]');xlabel('Time [s]');
% uiwait(hObject);
...
set(handles.pButton_addSccIdx,'Enable','off')
...
if DataStruct.desaccadedvel~=0
    ...
    if get(hObject,'Value')
        ...
        set(handles.pButton_addSccIdx,'Enable','on')
...
if get(hObject,'Value')
    ...
    plot(handles.vel_Axes,DataStruct.rawtime, DataStruct.omega_r,'r')  
    set(handles.vel_Axes,'nextplot','add');ylabel('dF/F0')
    
    cla(handles.pos_Axes)
    plot(handles.pos_Axes,DataStruct.rawtime, DataStruct.eyepos,'r')
    set(handles.pos_Axes,'nextplot','add');ylabel('dF/F0');xlabel('Time [s]');
...
%-->%Add Sccade%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- Executes on button press in pButton_addSccIdx.
function pButton_addSccIdx_Callback(hObject, eventdata, handles)
% hObject    handle to pButton_addSccIdx (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
DataStruct=get(handles.figure1,'UserData');

if DataStruct.desaccadedvel~=0
    set(handles.error,'String',{'Add saccades indices!'; '1) Select the start saccade index with one mouse left clik';'2) Select the start saccade index with one mouse left clik'});
    set(handles.error,'ForegroundColor','Black');
    
    idxSacc=DataStruct.saccExtremeIdx;
        
    [timeUser,~,button]=ginput(2); %time instants
    
    if all(button==1)
        newIDX(1,1)=find(DataStruct.rawtime<timeUser(1),1,'last'); %
        newIDX(1,2)=find(DataStruct.rawtime<timeUser(2),1,'last'); %
        
        idxSacc=[ idxSacc;newIDX];
        [t,IX]=sort(idxSacc(:,1));
        DataStruct.saccExtremeIdx=idxSacc(IX,:);
        %timeSacc=[timeSacc;timeUser'];
    end
