%CaDistribution_v5 
%plot the response Ca signal heatmap
clear
...
for nfile=1:size(signal,3)
    ...
    if CategorizeResponsive
        ...
        for trace=n
            ...
            SpikeTime{trace}=[SpikeTime{trace} time(min(locs{trace}(time(locs{trace})>StimInitial)))];%time of the first spike after stimulus
...
for file=1:nfile
    ...
    %% individual heatmap
    mean_im=cell(size(n));
    mean_BCim=cell(size(n));
    for trace=n
        for file=1:nfile
        %average heatmap
        heatmap_idx=time>SpikeTime{trace}(file)-0.03 & time<SpikeTime{trace}(file)+spikewindow-0.03;
        [BCim,im]=getim(FP{file},[FN{file}(1:11),'_OUT_MotCor.tif'],FrameRate); %get image file as matrix
        mean_im{trace}=cat(3,mean_im{trace},mean(im(:,:,heatmap_idx),3));
        mean_BCim{trace}=cat(3,mean_BCim{trace},mean(BCim(:,:,heatmap_idx),3));
        if 0 %show heatmap of individual sweep
            figure
            subplot(2,1,1)
            imagesc(mean_im{trace}(:,:,end));
            colorbar
            axis image;
            subplot(2,1,2)
            imagesc(mean_BCim{trace}(:,:,end));
            colorbar
            axis image;
            Fcaxis=[0 10];
            caxis(Fcaxis);
...
for trace=n
    ...
    subplot(2,1,1)  
    imagesc(mean(mean_im{trace}(:,:,SpikeGroup{trace}),3));
    colorbar
    axis image;
    Fcaxis=[0 4.*10^4];
    caxis(Fcaxis);
    title(['Cell ' num2str(trace) ' response']);
    
    subplot(2,1,2)
    imagesc(mean(mean_BCim{trace}(:,:,SpikeGroup{trace}),3));
    colorbar
    axis image;
    Fcaxis=[0 8];
    caxis(Fcaxis);
