...
% data=ans;
data.freq.eye_raw=data.leye_raw;
data.freq.time=data.time_l;
data.freq.SaccExtremeIdx=data.SaccExtremeIdxL;
...
data.freq.end_time_of_neg=[];
data.freq.beg_time_of_neg=[];
data.freq.end_time_of_pos=[];
data.freq.beg_time_of_pos=[];
...
if data.freq.eye_raw(data.freq.SaccExtremeIdx(i,1),1)-data.freq.eye_raw(data.freq.SaccExtremeIdx(i,2),1)>0;
    ...
    data.freq.beg_time_of_neg(i,1)=data.freq.SaccExtremeIdx(i,1);
...
elseif data.freq.eye_raw(data.freq.SaccExtremeIdx(i,1),1)-data.freq.eye_raw(data.freq.SaccExtremeIdx(i,2),1)<0;
    ...
    data.freq.beg_time_of_pos(i,1)=data.freq.SaccExtremeIdx(i,1);

end
end
%delete the zeros
data.freq.end_time_of_pos(data.freq.end_time_of_pos==0)=[];
data.freq.beg_time_of_pos(data.freq.beg_time_of_pos==0)=[];
data.freq.end_time_of_neg(data.freq.end_time_of_neg==0)=[];
data.freq.beg_time_of_neg(data.freq.beg_time_of_neg==0)=[];

data.freq.range_top=sort([data.freq.end_time_of_pos;data.freq.beg_time_of_neg]);
data.freq.range_bot=sort([data.freq.beg_time_of_pos;data.freq.end_time_of_neg]);

data.freq.range_top_Time=(data.freq.time(data.freq.range_top(:)))/60;
data.freq.range_bot_Time=(data.freq.time(data.freq.range_bot(:)))/60;
data.freq.range_top_eye_raw=data.freq.eye_raw(data.freq.range_top(:));
data.freq.range_bot_eye_raw=data.freq.eye_raw(data.freq.range_bot(:));
...
x1 = data.freq.time(data.freq.end_time_of_pos)/60;
y1 = data.freq.eye_raw(data.freq.end_time_of_pos);
plot(x1,y1,'*r');
title('left eyes raw traces');


x2 = data.freq.time(data.freq.end_time_of_neg)/60;
y2 = data.freq.eye_raw(data.freq.end_time_of_neg);
plot(x2,y2,'*g');
title('left eyes raw traces');
