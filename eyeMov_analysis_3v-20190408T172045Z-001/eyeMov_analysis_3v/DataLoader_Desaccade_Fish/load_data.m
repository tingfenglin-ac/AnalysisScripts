...
%             answer=questdlg(question,'Default Channels','yes','no','yes');
    ...
    if machine==4
        if not(isempty(Correspname)) %if is not emtphy we use tönnies, if not 3d turntable
            ...
            %Binocular
            %channels.RightEyeChannels=[35 42 41];%  Tor, vert, horiz
            %channels.RightEyeChannels1=[35 36 37];% Eyeseecam_precalibrated_channel
            %channels.LeftEyeChannels=[6 13 12];% Tor, vert, horiz
            %channels.LeftEyeChannels1=[6 7 8];% Eyeseecam_precalibrated_channel
            %channels.ChairChannels=[1 2 3];% drum and chair
            %channels.LaserChannels=[4 5]; %left led central led %[4 5]
            %channels.OtherChannels=[61 62 63];% Head inertial vel (roll, pitch, yaw)

            %With EvalDataColumns
            ...
            %channels.LeftEyeChannels=[6 13 12];% Tor, vert, horiz
            %channels.LeftEyeChannels1=[6 7 8];% Eyeseecam_precalibrated_channel
            %channels.ChairChannels=[63 64 65];% 
            %channels.LaserChannels=[66 67 68 62]; % LAser + left  Led 
            %channels.OtherChannels=[63];%[75 76 77];%[62]; Head inertial vel (roll, pitch, yaw)
            %alldata2=alldata;
            ...
            noADC=[];
            if sum(isfield(col,{'col.ADChannel5','col.ADChannel4','col.ADChannel3'}))<3
            ...
            if sum(isfield(col,{'col.ADChannel2','col.ADChannel1','col.ADChannel0','col.ADChannel6'}))<4
                channels.LaserChannels=[];
