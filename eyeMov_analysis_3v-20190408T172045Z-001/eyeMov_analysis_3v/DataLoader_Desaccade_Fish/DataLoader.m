...
%
DataStruct(1).OriginPath='..\..\..\Alcohol_GazeHolding_Data\Alcohol_Subjs\dataAlcoholSubjs_raw\BAC_010\';
...
while i<=length(DataStruct)
    if not(isempty(DataStruct(i).FileName)) %Eyesee cam do not use bin
...
if not(isempty(DataStruct(1).ReferenceFileName))
    if not(isempty(cellfilename))
        ...
        datacell= load_dataFR(cellfilename,[DataStruct(1).ReferencePathName,DataStruct(1).ReferenceFileName],DataStruct(1).polyFit,DataStruct(1).filter,Correspname,CorrespPath);
    ...
    errordlg('Select calibration file!');
end
set(handles.outputMsg,'string',sprintf('Ready! \n Waiting for user input'))
pause(0.01)
set(handles.figure1,'UserData',DataStruct);
...
[DataStruct(1).FileName,DataStruct(1).PathName] = uigetfile({'*.mat;','Matlab data (*.mat)';'*.bin','Labview binary (*.bin)';'*.*','All files'} ,'Select the data file', DataStruct(1).OriginPath);
...
if DataStruct(1).saveflag==[0 0];
    ...
    elseif strcmp(answer,'save')
        pushB_saveMat_Callback(hObject, eventdata, handles); %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        pushB_saveWSp_Callback(hObject, eventdata, handles)
