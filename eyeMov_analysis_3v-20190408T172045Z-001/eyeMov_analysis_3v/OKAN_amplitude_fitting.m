function [output]=OKAN_amplitude_fitting(output,time,value,start_time,end_time,okanstart_time,okanend_time)
...
fo= fitoptions('Method','NonlinearLeastSquares',...
               'Lower',[0,0,quantile(fit_value,0.1)],'Upper',[Inf,100,quantile(fit_value,0.9)],...
               'MaxFunEvals',100000,'TolFun',10^-12,...
           'StartPoint',[(abs(min(fit_value))),initTC,0]);
           
ft=fittype('-f*exp(-x/g)+h');
