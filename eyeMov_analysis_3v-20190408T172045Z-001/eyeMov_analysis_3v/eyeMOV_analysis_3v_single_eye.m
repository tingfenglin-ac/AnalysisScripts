currentFolder = pwd;
addpath ([currentFolder(1),':\eyeMov_analysis_3v\eyeMov_subroutine']);
[choice]=L_R_analysis();

eyeMOV_analysis_3v(choice)

function data=eyeMOV_analysis_3v(choice)
close all
...
data.bodyP_l=data_leye.bodyP;

%fake data
data.time_r=data_leye.rawtime;
data.reye_raw=1:100;
data.bodyP_r=1:100;
data.SaccAmplR=1:100;
data.SaccVelR=1:100;
data.SaccTimeR=1:100;
data.SaccFrqR=1:100;
data.SaccExtremeIdxR=1:100;
end
if length(choice)==1 && choice==2
...
data.bodyP_r=data_reye.bodyP; 

%fake data
data.time_l=data_reye.rawtime;
data.leye_raw=1:100;
data.bodyP_l=1:100;
data.SaccAmplL=1:100;
data.SaccVelL=1:100;
data.SaccTimeL=1:100;
data.SaccFrqL=1:100;
data.SaccExtremeIdxL=1:100;
end
if length(choice)==2
[data_leye]=Interactive_desaccader_fish(derivata(leye,1./fps),t,leye,bodyP,StimulusVelocity); %with 0 the checkbox substract eyePos average uses the whole signal
[data_reye]=Interactive_desaccader_fish(derivata(reye,1./fps),t,reye,bodyP,StimulusVelocity); % StimulusVelocity  the checkbox 'substract eyePos average' uses the first 5min of dark
