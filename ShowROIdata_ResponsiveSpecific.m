...
n=[1:16];
ROIgroup={[1:10]};  
ROIlabel=[{'Cell 1'} cellfun(@num2str,num2cell(36:40),'uni',0)];
increvalue=0.5;
    ...
    if CategorizeResponsive
        ...
        for trace=n
            if sum(SpikeGroup{trace})>0
                plot(time,permute(BC_signal(:,trace,SpikeGroup{trace})+incre(trace),[1,3,2]),'b');
            end
            hold on
            if sum(~SpikeGroup{trace})>0
                plot(time,permute(BC_signal(:,trace,~SpikeGroup{trace})+incre(trace),[1,3,2]),'g');
            end
        end
