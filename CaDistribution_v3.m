%CaDistribution_v3 20191218
%plot the spontaneous Ca signal heatmap
clear
...
for nfile=1:size(signal,3)
    ...
    if CategorizeResponsive
        ...
        for trace=n
            ...
            SponSpikelocs{trace}=[SponSpikelocs{trace};locs{trace}(time(locs{trace})<30)];%index of the pre stimulus spike
    ...
    %% plotting responsive and non responsive traces
        ...
        for file=1:nfile
            ...
            if SpikeGroup{trace}(file)
                figure(Tn)
                plot(time,permute(smoothBC_signal(:,trace,file)+incre(trace),[1,3,2]),'b');
                ...
                for pic=1:length(SponSpikelocs{trace})
                    ...
                    figure(Tn)
                    plot(time(heatmap_idx),smoothBC_signal(heatmap_idx,trace,file)+incre(trace),'k');
                    text(time(SponSpikelocs{trace}(pic)),smoothBC_signal(SponSpikelocs{trace}(pic),trace,file)+incre(trace)+0.05,num2str(pic))
            ...
            if ~SpikeGroup{trace}(file)
              figure(Tn)
                plot(time,permute(smoothBC_signal(:,trace,file)+incre(trace),[1,3,2]),'g');
                hold on
                area(time,Stim_signal*1000,...
                    'FaceColor',[1 0.5 0.5],...
                    'EdgeColor','none',...
