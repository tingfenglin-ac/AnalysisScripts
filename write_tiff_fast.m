...
%%
O = fopen(outfile,'w',endian_test);% Open the output file
if O==-1
    keyboard
end
...
%     pause(0.1);
    ...
    fwrite(O,out,sprintf('uint%i',tiff_fields.BitsPerSample.value),endian_test);% Call data and write
    minn = min([out(:);minn]);
    maxx = max([out(:);maxx]);
    clear out;
%     pause(0.05);
