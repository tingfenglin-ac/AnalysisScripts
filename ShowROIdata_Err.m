...
for i=1:size(signal,2)
    bin=0.5
    binN=round(time(end)./bin)
    for T=1:binN
        comb=combnk(1:size(BC_signal,3),2);
        for C=1:length(comb)
            BinData=BC_signal(time>T.*bin-bin &time<T.*bin,i,:);
            rho(C)=corr(BinData(:,comb(C,1)),BinData(:,comb(C,2)));
        end
        rhoMat(T,:)=rho;
    end
    plot((bin:bin:30)'-bin./2,mean(rhoMat,2)+incre(i),'k')
    hold on
    rhoErr=std(rhoMat,0,2);
    ErrArea_Smooth((bin:bin:30)'-bin./2,mean(rhoMat,2)+incre(i),rhoErr,...
        meancolor);
    plot([0 30],incre(i).*[1 1],':k')
end
%% plot area
% if plotArea
%     mn=2;
...
% %% plot area
% 
% signal_to_divide = ((mean(BC_signal(1:end,i,:),3))*100)+5
% if plotArea
%     mn=2;
%      figure;
%     mn=get(gcf,'Number');
...
%     set(gcf,'position',[1 1 defautPosition(3) 840]);
%      for i=1:size(signal,2)
% %          plot(time(1:end),ones(size(mean(BC_signal(1:end,i,:),3)))+incre(i),'color',linecolor,'linewidth',1.5);
%          hold on
%          signalStd=nanstd(BC_signal(1:end,i,:),0,3);
%          plot(time(1:end),incre(i)+(signal_to_divide./signalStd),'color',linecolor);
% %                   plot(time(1:end),incre(i)+(signalStd),'color','g');
% %          plot(time(1:end),incre(i)+signal_to_divide,'color','r');
% 
%      end
% end
%     
