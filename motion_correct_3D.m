function motion_correct_3D (vrargin)
...
%batch motion correction will start from the middle of the plains
mid=round(nfiles./2);
nplain=[mid:nfiles mid-1:-1:1];
for n = 1:nfiles
    ...
    chk = 0;
    while chk == 0
%         we always choose ENTER so skip this step
...
SL=[];
    ...
    % We are going to do a more efficient way since we are professional
    % scientist. Therefore, don't click the second time!!
%     if n ~=1
%         maxshift_x(n) = maxshift_x(n-1);
%         maxshift_y(n) = maxshift_y(n-1);
%     end
    %% Set Still Frames
    ...
    if n~=1
        info_ori = imfinfo(Filenames{n});
        Image(n).Still=zeros(info_ori.Height,info_ori.Width);
        %         im=double(load_tiffs_fast([Filenames{n-1}(1:end-4),'_MotCor.tif'],'nframes',nframes(n-1)));
        info_ref = imfinfo([Filenames{n-1}(1:end-4),'_MotCor.tif']);
        ydiff=round(info_ori.Height-info_ref.Height./2);
        xdiff=round(info_ori.Width-info_ref.Width./2);
        Image(n).Still(ydiff+(1:info_ref.Height),xdiff+(1:info_ref.Width))=...
            mean_image([Filenames{n-1}(1:end-4),'_MotCor.tif'], 'start_ind', 1, 'end_ind', 6.*frameperplain);
    end
%     if n<mid
%         info_ori = imfinfo(Filenames{n});
%         Image(n).Still=zeros(info_ori.Height,info_ori.Width);
...
%         xdiff=round(info_ori.Width-info_ref.Width)./2;
%         Image(n).Still(ydiff+(1:info_ref.Height),xdiff+(1:info_ref.Width))=...
%             mean_image([Filenames{n+1}(1:end-4),'_MotCor.tif'], 'start_ind', 1, 'end_ind', frameperplain);
%     end


%% Run Motion Correction Using Collected Parameters  
    motion_correct_shef(Filenames{n}, [Filenames{n}(1:end-4) '_MotCor.tif'],...
        'motcorr_crop_points',motcorr_crop_points(1,:),...
        'maxshift_x',maxshift_x(n),...
        'maxshift_y',maxshift_y(n),...
        'numframes', nframes(n),...
        'still_frame', Image(n).Still);
