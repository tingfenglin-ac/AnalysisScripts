...
[FileName,FolderPath] = uigetfile({'*.csv;*analysis.mat;*list.mat'},'Select an ROI file', 'Multiselect', 'on');
if iscell(FileName)
...
if length(ROIgroup)
 ...
 %% save the ROI analysis list of each condition
 % in the future, instead of open individual files one by one, a list file
 % could call all the individual file at once
    [file,path] = uiputfile('loc _list.mat','save list of ROI for a specific condition')
    if path~=0
...
% plot([618 618+31;618 618+31],[incre(1) incre(1);incre(end)+increvalue incre(end)+increvalue],'r');
